...
const SENSOR_TYPE_HIMIDITY: number = 1;
const SENSOR_TYPE_TEMPERATURE: number = 2;
const SENSOR_TYPE_H_T: number = SENSOR_TYPE_HIMIDITY + SENSOR_TYPE_TEMPERATURE;
const SENSOR_TYPE_VOLTAGE: number = 4;


const RELAY_MODE_AUTO: number = 0;
const RELAY_MODE_ALWAYS_ON: number = 1;
const RELAY_MODE_ALWAYS_OFF: number = 2;
...
interface sensor_data {
    sensor_type: number, himidity: number, temperature: number, voltage: number
}

interface sensor_packet {
    sensor_id: number,
...
interface RelayConfiguration {
    ...
    relay_id: number,
    mode: number,
    auto_dependence: number,
    min_value: number,
    max_value: number,
}

function compare_sensor_packets(a: sensor_packet, b: sensor_packet): number {
    return a.sensor_id - b.sensor_id;
}

function getSensors(sensors_json: string) {

    const connected_sensors = JSON.parse(sensors_json) as Array<sensor_packet>;
...
function generateSensorWidget(sensor_packet: sensor_packet) {
    var html_string =
        ` <li><div class="sensor_value_container">
...
var WidgetMap: Record<number, HTMLLIElement> = [];

function add_new_to_list(new_sensor: sensor_packet) {
    const main_list = document.getElementById('main_list') as HTMLUListElement;
...
function disable_widget(widget: HTMLLIElement) {
...
function updateSensorsWidget(sensors: Array<sensor_packet>) {
    ...
    fetch("/get_sensors").then((res) => {
        ...
        res.text().then((val) => {
            ...
            for (var i = 0; i < sensors.length; i++) {
                ...
                const voltage_a = document.getElementById('sensor' + sensors[i].sensor_id.toString() + '_voltage') as HTMLAnchorElement;
...
function enable_ranges_list(id: string) {
...
function disable_ranges_list(id: string) {
    const element = document.getElementById(id);
    if (element) {
...
function update_relay_status(relay_number: number, current_mode: number, current_status_is_ON: boolean) {

    const mode_elem = document.getElementById('relay' + relay_number + '_mode') as HTMLAnchorElement;
    const status_elem = document.getElementById('relay' + relay_number + '_status') as HTMLAnchorElement;
...
function get_relay_configuration(relay_number: number): RelayConfiguration {
    let relay_conf: RelayConfiguration = { sensor_id: 0, relay_id: 0, mode: 0, min_value: 0, max_value: 0, auto_dependence: 0 };
    ...
    const sensor_sel_element = document.getElementById('relay_conf_sensor_sel_' + relay_number.toString()) as HTMLSelectElement;
    const sensor_dep_element = document.getElementById('relay_conf_dep_sel_' + relay_number.toString()) as HTMLSelectElement;
    const min = document.getElementById('min' + relay_number.toString()) as HTMLInputElement
    const max = document.getElementById('max' + relay_number.toString()) as HTMLInputElement
...
function SetRange(n: number) {
    ...
    if (relay_mode.textContent == "val_auto") {
        ...
        const min = document.getElementById('min' + n.toString()) as HTMLInputElement
        const max = document.getElementById('max' + n.toString()) as HTMLInputElement
